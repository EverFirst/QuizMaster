...
export async function generateQuizQuestion(category: string, type: string = "multiple_choice"): Promise<QuizQuestion> {
  ...
  const prompt = categoryPrompts[category as keyof typeof categoryPrompts] || categoryPrompts.general;
...
6. JSON 형식으로 응답해주세요: {"question": "빈칸이 포함된 문제내용", "correctAnswers": ["정답1", "정답2", "정답3"], "hints": ["힌트1", "힌트2"]}`
    : `당신은 한국어 퀴즈 문제 제작 전문가입니다. 다음 조건에 맞는 객관식 문제를 생성해주세요:
1. 문제는 명확하고 이해하기 쉬워야 합니다
2. 4개의 선택지를 제공해주세요
3. 정답은 하나만 있어야 합니다
4. 난이도는 일반인이 도전할 만한 수준으로 해주세요
5. JSON 형식으로 응답해주세요: {"question": "문제내용", "options": ["선택지1", "선택지2", "선택지3", "선택지4"], "correctAnswer": 정답번호(0-3)}`;
